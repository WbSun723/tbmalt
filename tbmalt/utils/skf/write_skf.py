...
class WriteSK:
    ...
    def __init__(self, path: str, element: list, **kwargs):
    ...
    def __call__(self, mode='w', **kwargs):
        ...
        if self.sk_type == 'normal':
            self._write_sk_normal(mode, **kwargs)
        elif self.sk_type == 'compression_radii':
            self._write_sk_compression_radii()

    def _write_sk_normal(self, mode, **kwargs):
        """Read skf data from SKF files and write into h5py binary data."""
        repulsive = kwargs.get('repulsive', False)
        with h5py.File(self.output_name, mode) as f:
    ...
    def _write_sk_compression_radii(self, mode, type_compression_sk='all'):
        """Read .skf data from skgen with various compR."""
        raise NotImplementedError("Not implement compression radii writing.")
